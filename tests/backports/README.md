...
freshly made UUIDs in its header on every run.
    ...
    sh host/scenekit/lighting/cases.sh  writes device/scenekit-lighting-expectations.h: macOS SceneKit's pixel for 1276 lighting cases, and a known-wrong renderer's for the negative control of device/scenekit-lighting.m
...
- `scenekit-lighting.m` (`scenekit-lighting-expectations.h`): the `SCNView` shader over the lighting grid, one pixel per case, within one level of macOS SceneKit's pixel (`host/scenekit/lighting/cases.sh`); its negative control, the pixels of a renderer with the specular exponent or the roughness a little off, must be missed. Needs a device.
