...
- `dynamics-watch.m`: a command-line program of the shipped `UIDynamicAnimator`'s watch of its reference view, on an emulated 6.1.3 with `charon@apple-backports` linked: which changes of the view wake a resting animator, and that no KVO leak line is logged when the animator or the view is released first (`facts/UIKit/UIDynamicAnimator.md` M3).
