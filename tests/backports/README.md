...
- `uiview-kvo.m`: a command-line probe of what KVO of a `UIView`'s `frame` and `bounds` fires on nine ways of changing a view's geometry, beside the calls its two setters get, and of what KVO does with an observer left on a view that deallocates (`facts/UIKit/UIDynamicAnimator.md` M3). Runs in `xmake emulate` on 4.3 (`-d iPhone3,1`, linking arclite with `-fobjc-arc` on the link line), 5.1.1, 6.0 and 6.1.3; path (f) is skipped where `NSLayoutConstraint` is absent (below 6.0); it is not an app, so it needs no SpringBoard.
- `dynamics-watch.m`: a command-line program of the shipped `UIDynamicAnimator`'s watch of its reference view, on an emulated 4.3, 5.1.1, 6.0 and 6.1.3 with `charon@apple-backports` linked (the target waives `charon.waive.weak-imports` with its reason below 6.1.3): which changes of the view wake a resting animator, and that no KVO leak line is logged when the animator or the view is released first (`facts/UIKit/UIDynamicAnimator.md` M3). A bare 4.3 process has no `CADisplay`, so there the wake checks are skipped with a line each and the lifetimes run whole.
