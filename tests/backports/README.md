...
- `uiview-kvo.m`: a command-line probe of what KVO of a `UIView`'s `frame` and `bounds` fires on nine ways of changing a view's geometry, beside the calls its two setters get, and of what KVO does with an observer left on a view that deallocates (`facts/UIKit/UIDynamicAnimator.md` M3). Runs in `xmake emulate -d iPhone4,1 -r 6.1.3`; it is not an app, so it needs no SpringBoard.
