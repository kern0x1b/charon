...
#import "textsystem7-expectations.h"
...
@interface TextSystem7Delegate : UIResponder <UIApplicationDelegate>
@property (nonatomic, strong) UIWindow *window;
@end

@implementation TextSystem7Delegate

- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)options
{
    [[NSFileManager defaultManager] createDirectoryAtPath:results_folder withIntermediateDirectories:YES attributes:nil error:NULL];
    [[NSFileManager defaultManager] removeItemAtPath:[results_folder stringByAppendingPathComponent:@"textsystem7.done"] error:NULL];
    charon_log_to([results_folder stringByAppendingPathComponent:@"textsystem7.log"]);
    self.window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    self.window.rootViewController = [[UIViewController alloc] init];
    [self.window makeKeyAndVisible];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSDictionary *expected = [NSJSONSerialization JSONObjectWithData:[NSData dataWithBytes:textsystem7_expectations length:strlen(textsystem7_expectations)] options:0 error:NULL];
        NSMutableDictionary *records = [NSMutableDictionary dictionary];
        textsystem7_run(^(NSString *name, NSString *value) {
            records[name] = value;
        ...
        [summary writeToFile:[results_folder stringByAppendingPathComponent:@"textsystem7.done"] atomically:YES encoding:NSUTF8StringEncoding error:NULL];
    });
    return YES;
}
...
{
    @autoreleasepool {
        return UIApplicationMain(argc, argv, nil, NSStringFromClass([TextSystem7Delegate class]));
    }
}
