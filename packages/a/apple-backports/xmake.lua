package("apple-backports")
    ...
    add_configs("avkit", {description = "Build libAVKitBackports.dylib, for an application that floats a video mini-player through AVPictureInPictureController; AVKit.framework does not exist at all on this release, and neither does any cross-app window-compositing surface a third-party dylib could draw into, so this is an in-app floating window only - the real AVPlayerLayer is reparented into a draggable window above the host app's own view hierarchy, keeps playing for real, and is reparented back on -stopPictureInPicture. It does not persist once the host app leaves the foreground, the one dimension a real SpringBoard-side companion (the kind CallKitBackports' CharonCallScreen already models for its own call screen) would be needed for and this port does not build.", default = false, type = "boolean"})

    on_load("iphoneos", function (package)
        package:add("links", table.join(package:config("uikit") and {"UIKitBackports"} or {}, package:config("corelocation") and {"CoreLocationBackports"} or {}, package:config("coredata") and {"CoreDataBackports"} or {}, package:config("security") and {"SecurityBackports"} or {}, package:config("avfoundation") and {"AVFoundationBackports"} or {}, package:config("webkit") and {"WebKitBackports"} or {}, (package:config("graphics") or package:config("avfoundation")) and {"GraphicsBackports"} or {}, package:config("localauthentication") and {"LocalAuthenticationBackports"} or {}, (package:config("safariservices") or package:config("authenticationservices")) and {"SafariServicesBackports"} or {}, package:config("authenticationservices") and {"AuthenticationServicesBackports"} or {}, package:config("backgroundtasks") and {"BackgroundTasksBackports"} or {}, package:config("photos") and {"PhotosBackports"} or {}, package:config("gamecontroller") and {"GameControllerBackports"} or {}, package:config("vision") and {"VisionBackports"} or {}, package:config("metal") and {"MetalBackports"} or {}, package:config("metalkit") and {"MetalKitBackports"} or {}, package:config("coretelephony") and {"CoreTelephonyBackports"} or {}, package:config("accelerate") and {"AccelerateBackports"} or {}, package:config("callkit") and {"CallKitBackports"} or {}, package:config("contacts") and {"ContactsBackports"} or {}, package:config("corespotlight") and {"CoreSpotlightBackports"} or {}, package:config("pushkit") and {"PushKitBackports"} or {}, package:config("opengles") and {"OpenGLESBackports"} or {}, package:config("javascriptcore") and {"JavaScriptCoreBackports"} or {}, package:config("scenekit") and {"SceneKitBackports"} or {}, package:config("mediaplayer") and {"MediaPlayerBackports"} or {}, package:config("avkit") and {"AVKitBackports"} or {}, {"FoundationBackports"}))
    end)
    ...
    on_install("iphoneos", function (package)
        ...
        local libraries = table.join({"FoundationBackports"}, package:config("uikit") and {"UIKitBackports"} or {},
                                     ...
                                     (package:config("graphics") or package:config("avfoundation")) and {"GraphicsBackports"} or {},
