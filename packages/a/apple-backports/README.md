...
`CSSearchableIndex`, `CSSearchableItem` and `CSSearchableItemAttributeSet` are carried in `libCoreSpotlightBackports.dylib`, built with the `corespotlight` config, for the two applications of the corpus that strong-import them: the classes are real, `CSSearchableItem` generates a UUID when none is given and defaults its expiration a month out as the header says, and `CSSearchableItemAttributeSet` carries the 19 properties of the `CSGeneral` category with real storage, `NSCopying` and `NSSecureCoding`; the later Images, Media, Documents, Events, Places, Messaging and custom-key categories are not. iOS 6.1.3 has no CoreSpotlight and no system Spotlight index an application can write to - only the private `Spotlight.framework`'s `SPSpotlightManager`, a pull model the system calls into the app through, which does not compose with `CSSearchableIndex`'s push writes - so `isIndexingAvailable` answers NO and every write method answers the error iOS itself gives for a device without indexing, `CSIndexErrorCodeIndexingUnsupported`, never a silent success. `CSSearchableItemActionType` and `CSSearchableItemActivityIdentifier` are carried at the release's own values, read out of the arm64 shared cache of iOS 12.0, though neither is ever posted since nothing of this port's making is ever surfaced by the system's Spotlight to be tapped. `facts/CoreSpotlight/CoreSpotlight.md`.
