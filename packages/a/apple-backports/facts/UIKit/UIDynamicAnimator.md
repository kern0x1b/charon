...
0.002 m, edge or chain a fixed 0.02 m. Measured on the host (T5, §12): in the host build (no skin, full-size box) the port rests at 248.216629, the oracle's to the digit; as shipped (sub-step 0.004 s, box 1 pt inside, skin 0.001 m) the same scene rests at 249.0188141, its bottom 0.98 pt above the floor. That value is the port's own and is pinned to 1e-3 pt by `landing_ios70_test.m` ("T5 as shipped"); it is not a 7.0 measurement, none exists. The 0.8022 pt between the builds is split by three host runs, one shipped constant changed at a time from the host's build: the box inset alone +1.0028 (249.2194214), the sub-step alone -0.2090 (248.0075836), the skin (0.001 m) alone 0.0000 (248.2166290); the three sum to +0.7937, and the 0.0084 left over is the two together, not a fourth constant (shipped minus the sub-step is 249.2194214 too, minus the box 248.0075836, minus the skin 249.0188141: the box adds +1.0112 on the shipped sub-step, the sub-step takes -0.2006 off on the shipped box). Why the skin does nothing here is not explained: a skin of 0.01 m on the host's build also leaves 248.2166290, 0.05 m gives 240.0818787. The sub-step also moves the fall (frame 26: 192.0832977 host, 194.1759491 shipped). The scene of `device/dynamics.m` (a 40x40 view, the reference bounds as the boundary, a loop 1 pt outside them) rests as shipped at a body y of 460.1338806, 0.13 pt past the bounds' bottom, so the view's bottom edge is at the bounds' bottom after the animator's rounding onto the screen's grid: gap 0, not 1 pt (the host's build there gives 0.9987). The earlier "about 1 pt per side, 2.5 pt at rest" was a bound, not a measurement.
...
**The shipped watch, on 4.3, 5.1.1, 6.0 and 6.1.3** (`tests/backports/device/dynamics-watch.m`, `charon@apple-backports` with `uikit` linked and `apple_minimum` set to each release, `xmake emulate -d iPhone3,1 -r 4.3|5.1.1|6.0` and `-d iPhone4,1 -r 6.1.3`; the probe's own observer above is not the port's, this runs `CharonReferenceViewWatch`; the target waives `charon.waive.weak-imports`, since the release refuses the libraries' weak imports of `NSProgress`, `NSUUID`, `NSTextContainer`, `NSByteCountFormatter` and, on 4.3, `CFRunLoopObserverCreateWithHandler`, none of which this program reaches): on 5.1.1, 6.0 and 6.1.3 a resting animator wakes on (a) `view.frame`, (b) `view.bounds` and (e) autoresizing, and not on (c) layer bounds, (g) center or (h) transform; with the animator released first and then the view, with the view released first and the animator alive (its `referenceView` is nil after), and with two animators over one view of which the first is released (the second still wakes), the view deallocates each time when the run says it does (a counting subclass) and the log has no `was deallocated while key value observers were still registered` line and no other line of the animator's; 400 views made after saw nothing. 0 failed checks on all three, so 5.1.1, where a leaked observer does damage (the probe's control above), leaves nothing behind. On 4.3 the lifetimes are the same and clean, 0 failed checks and no such line; the wake checks are not made there: `-[CADisplay mainDisplay]` is nil in a bare process on the emulated 4.3 and `+[CADisplayLink displayLinkWithTarget:selector:]` faults in QuartzCore (`+0x9680e`, a fault at address 4), which a program that does nothing but ask for a display link does as well, so the animators are kept from starting a display link there and the program prints seven `skip` lines. What the run found, and the port's fix: below 6.0 the animator called `-constraints` and `-setTranslatesAutoresizingMaskIntoConstraints:` on the item's view (`unrecognized selector`, the first `addBehavior:` on 4.3 and 5.1.1), now behind `respondsToSelector:`; and an animator released while behaviors were added on 4.3 was autoreleased by the behaviors' `-charon_context` inside its own `-dealloc` (retained and autoreleased, no handshake below 5.0 cancels the pair), so the pool later released freed memory (`__NSAutoreleaseFreedObject(): release of previously deallocated object`), now released in a pool of the dealloc's own; measured clean on 4.3 for gravity, push, snap, both attachments, both collisions, an item behavior and a composite with children. Both were seen with no watch involved (a view and an animator with no behavior did neither). Not measured: (f) on the shipped class, and the wake checks on 4.3 (a device would run them in an app).
