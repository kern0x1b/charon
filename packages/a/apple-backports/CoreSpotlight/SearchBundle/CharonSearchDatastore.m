...
// it, so printf/NSLog to this process's own stdout/stderr reaches nobody - a file is the one
// channel proven to survive that boundary (the same one CTCellularData9.m's device test used
// earlier this session). Appends only; never assume the channel works without a line like this one
...
static void CharonSearchDatastoreRegister(void)
{
    ...
    CharonSearchDatastoreLog(@"objc_registerClassPair done, CharonSearchDatastore is live");
}
