task("firmware")
    ...
    on_run("main")
    set_menu {
        usage = "xmake firmware [--arch=ARCH] fetch RELEASE | xmake firmware --device=IDENTIFIER rootfs RELEASE | xmake firmware [--arch=ARCH] classes RELEASE | xmake firmware list",
        ...
        options = {
            ...
            {"o", "output", "kv", nil, "Where classes writes its JSON (default: beside the release's cache)."},
            {nil, "action", "v", nil, "fetch, rootfs, classes or list."},
