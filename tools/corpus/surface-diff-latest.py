...
def framework_headers_dir(sdk, framework, target):
    """Where framework's own Headers/ live, checked in the same order clang itself would
    resolve <Framework/...> for this target -- the iOSSupport overlay first for a macabi
    target, then the SDK's native System/Library/Frameworks. A framework bundle's Headers is
