...
# CACHES and STORE are DATA (an accumulated ~870 MB cache ladder and a ~4 MB ingest store), not
# this module's own location -- HERE would put them wherever this file is copied to, which broke
# silently the moment this module moved from coordination/corpus-tools/ to charon/tools/corpus/
# (2026-09-23): every caller that trusts agg.STORE/agg.CACHES without its own override (gen-report.py,
# live-scan.py's fcs()) would otherwise look for store.json beside THIS file and find nothing. Both
# live in the durable location CHARON_CORPUS_ROOT addresses (default coordination/corpus/); either
# can still be overridden per-directory for a one-off run. See tools/corpus/README.md.
...
if not os.path.isdir(CORPUS):
    sys.exit("aggregate.py: no corpus/ data directory found at %s. Set CHARON_CORPUS_ROOT to the "
              "directory that contains corpus/ (holds store.json, caches/)." % CORPUS)
CACHES = os.environ.get("CHARON_CACHES_DIR") or os.path.join(CORPUS, "caches")
STORE = os.path.join(CORPUS, "store.json")
